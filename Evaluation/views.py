...
from PlagCheck.views import render_plagcheck_suspicion_list, render_plagcheck_compare
...
def evaluation(request, course_short_title=None):
    ...
    if selection not in ('error', 'questions', 'plagcheck_suspicions'):
        ...
        overview = render_plagcheck_suspicion_list(request, course)
...
def plagcheck_suspicions(request, course_short_title=None):
    ...
    return render_to_response('evaluation.html', {
            'overview': suspicions_html,
            'course': course,
            'stabilosiert_plagcheck_suspicions': 'stabilosiert',
            'count_plagcheck_suspicions': request.session['count'],
            'selection': request.session['selection'],
        },
        context_instance=RequestContext(request))
...
def plagcheck_compare(request, course_short_title=None, suspicion_id=None):
    ...
    plagcheck_compare_html = render_plagcheck_compare(request, course, suspicion_id)

    return render_to_response('evaluation.html', {
            'detail_html': plagcheck_compare_html,
            'course': course,
            'stabilosiert_plagcheck_suspicions': 'stabilosiert',
            'count_plagcheck_suspicions': request.session['count'],
        },
